...
- **题库管理**：单选、多选、判断、填空、简答五种题型，支持难度、知识点、分值与解析；填空/简答可维护「要点名称 + 分值」评分点，各评分点之和即为题目总分；支持 JSON / Excel 批量导入。
- **智能组卷**：教师按题型数量、分值、难度自动抽题组卷；不同学生的题目顺序与选项顺序随机打乱；组卷时快照评分点（当卷面分值与题库分值不同会等比折算并保证合计一致），此后修改题库不影响已有答卷的批改口径。
- **在线考试**：倒计时自动交卷、标记疑问、答题卡导航、刷新页面视为交卷。
- **自动阅卷与成绩分析**：客观题自动判分，填空/简答由教师按评分点在线录分并自动汇总，超过单个评分点或题目满分时拒绝保存；成绩报告含题型得分分布、评分点得分/失分明细（未批改显示待批）、正确率与排名；旧题无评分点的继续按整题给分。
